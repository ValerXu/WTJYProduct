...
public class ModelSearchEditText_Clear extends android.support.v7.widget.AppCompatEditText {
    ...
    public ModelSearchEditText_Clear(Context context) {
        super(context);
        init();
        // 初始化该组件时，对EditText_Clear进行初始化 ->>步骤2
    }

    public ModelSearchEditText_Clear(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ModelSearchEditText_Clear(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     */
    private void init() {
        clearDrawable = getResources().getDrawable(R.drawable.clear1);
        searchDrawable = getResources().getDrawable(R.drawable.search1);
