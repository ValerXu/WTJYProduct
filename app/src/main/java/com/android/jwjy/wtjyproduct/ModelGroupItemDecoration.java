...
public class ModelGroupItemDecoration<Group, Child extends ClassBean> extends RecyclerView.ItemDecoration {
    ...
    public ModelGroupItemDecoration() {
        super();
        init();
    }
...
//        onDrawGroup(c, parent);   //画分组标签（功能咱不需要）
    }
    ...
    protected void onDrawOverGroup(Canvas c, RecyclerView parent) {
        ...
        String data = groupTitle.substring(0,groupTitle.indexOf(";"));
        String data1 = groupTitle.replace(data + ";" ,"");
        c.drawText(data, x, y, mTextPaint);
        c.drawText(data1, parent.getMeasuredWidth() - mPaddingRight - getTextLenghtPx(data1), y, mTextPaint);
        if (isRestore) {
            //还原画布为初始状态
            c.restore();
        }
     ...
     */
    public void notifyDataSetChanged(ModelGroupRecyclerAdapter<Group, Child> adapter) {
        mGroup.clear();
