...
public class ControllerSelectPictureDialog extends DialogFragment {
	private static final String TAG = ControllerSelectPictureDialog.class.getSimpleName();
	...
	public static ControllerSelectPictureDialog getInstance(Context mContext, Bundle bundle)
	{
		ControllerSelectPictureDialog selectPictureDialog = new ControllerSelectPictureDialog();
	...
	private void initEvent() {
		...
		mConfirmTv.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View view) {
				//返回界面，并传递值回去
				ArrayList<ControllerPictureBean> selectedPhotoList = new ArrayList<ControllerPictureBean>();
				...
				if(maxCount <= 1){
					...
					pictureBean.setPicName(ControllerGlobals.getFileName(imgPath));
	...
	private void backWithResult(ArrayList<ControllerPictureBean> photosList) {
		dismiss();
	...
	public interface OnSelPicDialogConfirmClickListener {
		void onConfirmClick(ArrayList<ControllerPictureBean> selectedPhotoList);
	}
}
